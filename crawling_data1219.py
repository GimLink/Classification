...
# 키워드 가져오기
keys = pd.read_csv("Python/1219/keyword.txt", encoding="utf-8", names=['keyword'])
"""
    keyword
0     오렌지
1     한라봉
...
def image_download(keyword):
    # image download 함수
    create_folder('Python/1219/' + keyword + '_high_resolution')

    # chromdriver 가져오기
    options = webdriver.ChromeOptions()
    options.add_experimental_option("detach", True)
    driver = webdriver.Chrome("Python/1219/chromedriver", options=options)
    # windows
    ...
    for k, i in enumerate(links):
        try:
            url = i
            start = time.time()
            urllib.request.urlretrieve(url, 'Python/1219/' + keyword + '_high_resolution/' + keyword + '_' + str(k-forbidden) + '.jpg')
...
if __name__ == '__main__':
    pool = Pool(processes=2)  # 5개의 프로세스를 사용합니다.
